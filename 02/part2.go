...
func main() {
	...
	for i := range list_of_reports {
		...
		if !is_safe {
			for j := range len(list_of_reports[i]) {
				report := delete_from_slice(list_of_reports[i], j)
...
func delete_from_slice(slice []int, elem_idx int) []int {
	var new_slice []int

	new_slice = append(new_slice, slice[:elem_idx]...)
	new_slice = append(new_slice, slice[elem_idx+1:]...)
	return new_slice
}
